...
//------------------------------------------Armement des signaux-------------------------------------------------------
  ...
  //initialise le semaphore pose probleme
  ...
  printf("(SERVEUR %d) fdPipel[0] = %d fPipe[1] : %d\n",getpid(),fdPipe[0],fdPipe[1]);
...
//--------------------------------Etape3------------------------------------------------------------
      ...
      case UPDATE_PUB :  // TO DO Envoie de la pub a tout les processeus  client connecte
                      i = 0;
...
void afficheTab()
{
  ...
  for (int i=0 ; i<6 ; i++)
  fprintf(stderr,"%6d -%20s- %6d\n",tab->connexions[i].pidFenetre,tab->connexions[i].nom,tab->connexions[i].pidCaddie);
  fprintf(stderr,"\n");
}
...
void handlerSIGINT(int) 
{
  ...
  while(i < 0)
  {
    if(tab->connexions[i].pidCaddie != 0)  
    {
      kill(tab->connexions[i].pidCaddie, SIGKILL); 
    }
  }
  ...
  fprintf(stderr,"(SERVEUR %d) Suppression du Semaphore\n\n",getpid());

  //supprime file de messages
  if(msgctl(idQ,IPC_RMID,NULL) == -1)
  {
    perror("Erreur de msgctl");
    exit(1);
  }
  fprintf(stderr,"(SERVEUR %d) Suppression du File de Message\n\n ",getpid());

  //supprime memoire partage
  if(shmctl(idShm,IPC_RMID,NULL) == -1)
  {
    perror("Erreur de shmctl");
    exit(1);
  }
  fprintf(stderr,"(SERVEUR %d) Suppression de la Memoire partagée\n\n ",getpid());
...
void HandlerSIGCHLD(int)
{
  ...
  fprintf(stderr, "\n(SERVEUR) Suppressionr du fils %d de la table des processus\n", id);
